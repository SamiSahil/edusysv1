...
export async function renderExamsPage() {
    ...
    const renderExamScheduleTab = () => {
        ...
        document.querySelectorAll('.delete-exam-btn').forEach(btn => btn.addEventListener('click', (e) => { 
            // --- THIS IS THE FIX ---

            // 1. Immediately capture the exam ID from the initial click event.
            const examIdToDelete = e.currentTarget.dataset.id;

            // 2. Pass a callback function to the modal that uses the captured ID.
            showConfirmationModal('Are you sure?', async () => { 
                // 3. Now, instead of using the old 'e', we use the reliable 'examIdToDelete' variable.
                if (await apiService.remove('exams', examIdToDelete)) { 
