...
const getStudents = asyncHandler(async (req, res) => {
    ...
    if (loggedInUser.role === 'Admin') {
        ...
        students = await populateStudentDetails(Student.find({}));
    } else if (loggedInUser.role === 'Teacher') {
        // Teacher gets only students from sections they are assigned to.
        const classTeacherSections = await Section.find({ classTeacherId: loggedInUser.teacherId }).select('_id');
        const classTeacherSectionIds = classTeacherSections.map(s => s._id);

        const timetableSections = await Timetable.find({ teacherId: loggedInUser.teacherId }).distinct('sectionId');
        
        const allMySectionIds = [...new Set([...classTeacherSectionIds.map(String), ...timetableSections.map(String)])];

        students = await populateStudentDetails(Student.find({ 'sectionId': { $in: allMySectionIds } }));
    } else {
        // No other roles should access this list endpoint.
        students = [];
    }
...
// Other functions like create, update, delete are already protected by 'authorize' middleware in the routes file.
// So, no changes are needed for them unless you need more granular, object-level checks.
